...
/**
 * История последних просканированных товаров v2: пишет её сервер на успешном поиске
 * по штрихкоду, отдельного вызова «залогируй скан» у приложения нет.
 */
class ProductScanTest extends TestCase
{
    ...
    public function test_scanning_the_same_product_twice_bumps_the_row_instead_of_duplicating_it(): void
    {
        ...
        $this->asDevice($seller)->getJson('/api/v2/products/8011003993802')->assertOk();
        $first = ProductScan::sole();

        $this->travel(2)->minutes();
        $this->getJson('/api/v2/products/8011003993802')->assertOk();
    ...
    public function test_the_history_returns_the_newest_scan_first(): void
    {
        ...
        $this->asDevice($this->seller())->getJson('/api/v2/products/8011003993802')->assertOk();
        $this->travel(1)->minutes();
        $this->getJson('/api/v2/products/8011003993819')->assertOk();

        $rows = $this->getJson('/api/v2/products/recent')
            ->assertOk()
    ...
    public function test_the_history_limit_comes_from_the_request_and_is_capped(): void
    {
        ...
        $this->asDevice($seller)
            ->getJson('/api/v2/products/recent?limit=2')
            ->assertOk()
            ->assertJsonCount(2, 'data')
            ->assertJsonPath('meta.limit', 2);

        $this->getJson('/api/v2/products/recent?limit=999')
            ->assertOk()
    ...
    {
        $this->asDevice($this->seller())
            ->getJson('/api/v2/products/recent?limit=vsyo')
            ->assertStatus(422)
    ...
    public function test_the_history_belongs_to_the_seller_not_to_the_shop(): void
    {
        ...
        $this->asDevice($mine)
            ->getJson('/api/v2/products/recent')
            ->assertOk()
    ...
    public function test_a_product_hidden_after_the_scan_leaves_the_history(): void
    {
        ...
        $this->asDevice($seller)->getJson('/api/v2/products/8011003993802')->assertOk();
        $this->getJson('/api/v2/products/recent')->assertOk()->assertJsonCount(1, 'data');

        $product->update(['status' => 'hidden']);

        $this->getJson('/api/v2/products/recent')->assertOk()->assertJsonCount(0, 'data');
        $this->assertDatabaseCount('product_scans', 1);
    }
    ...
    public function test_the_history_respects_the_field_policy(): void
    {
        ...
        $row = $this->getJson('/api/v2/products/recent')->assertOk()->json('data.0.product');
    ...
    public function test_the_seller_can_clear_the_history(): void
    {
        ...
        $this->asDevice($seller)
            ->deleteJson('/api/v2/products/recent')
            ->assertOk()
    ...
    /**
     * Чистится история сотрудника, а не всей смены: соседний продавец своих сканов
     * не лишается.
     */
    public function test_clearing_the_history_touches_only_this_seller(): void
    {
        $mine = $this->seller();
        $other = $this->seller();
        ProductScan::factory()->create(['user_id' => $mine->id]);
        ProductScan::factory()->create(['user_id' => $other->id]);

        $this->asDevice($mine)->deleteJson('/api/v2/products/recent')->assertOk();

        $this->assertDatabaseCount('product_scans', 1);
        $this->assertDatabaseHas('product_scans', ['user_id' => $other->id]);
    }

    public function test_the_history_is_not_public(): void
    {
        $this->getJson('/api/v2/products/recent')->assertUnauthorized();
        $this->deleteJson('/api/v2/products/recent')->assertUnauthorized();
    }
}
